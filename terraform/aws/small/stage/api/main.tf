...
module "public_api_route" {
  count              = try(var.public_endpoints, false) && (length(keys(var.public_endpoints)) > 0) ? 1 : 0
...
module "private_api_route" {
  count              = try(var.private_endpoints, false) && (length(keys(var.private_endpoints)) > 0) ? 1 : 0
