...
buildDir=$(jq -r ".lambda.build.src.buildDir" <./xilution.json)
if [[ "${buildDir}" == "null" ]]; then
  echo "Unable to find build directory."
  exit 1
fi

commands=$(jq -r ".lambda.build.src.commands[] | @base64" <./xilution.json)
...
layerDir=$(jq -r ".lambda.build.layer.layerDir" <./xilution.json)
if [[ "${layerDir}" == "null" ]]; then
  echo "Unable to find layer directory."
  exit 1
fi

commands=$(jq -r ".lambda.build.layer.commands[] | @base64" <./xilution.json)
execute_commands "${commands}"
